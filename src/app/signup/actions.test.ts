...
describe('login server action', () => {
    ...
    it('logs error if signUp fails', async () => {
        ...
        const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation(() => {})
        await signup(formData)
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error signing up:', 'signup error')
        expect(mockRedirect).toHaveBeenCalledWith('/')
        consoleErrorSpy.mockRestore()
    })

    it('creates profile and assigns role on successful signup', async () => {
        mockSignUp.mockResolvedValue({ data: { user: { id: 'user123' } }, error: null })
        mockInsert
        ...
        expect(mockSupabase.insert).toHaveBeenNthCalledWith(
            1,
            [{ user_id: 'user123', firstname: 'John', lastname: 'Doe', birthday: '2000-01-01', country: 'Italy' }]
        )
        expect(mockSupabase.insert).toHaveBeenNthCalledWith(
            2,
            [{ user_id: 'user123', role: 'user' }]
        )
        expect(consoleLogSpy).toHaveBeenCalledWith('Profile created successfully:', 'profileData')
        expect(consoleLogSpy).toHaveBeenCalledWith('Role assigned successfully:', 'roleData')
        expect(mockRedirect).toHaveBeenCalledWith('/')
    ...
    it('logs error if profile creation fails', async () => {
        ...
        const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation(() => {})
        await signup(formData)
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error creating profile:', 'profile error')
        expect(mockRedirect).toHaveBeenCalledWith('/')
        consoleErrorSpy.mockRestore()
    })
    ...
    it('logs error if role assignment fails', async () => {
        ...
        const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation(() => {})
        await signup(formData)
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error assigning role:', 'role error')
        expect(mockRedirect).toHaveBeenCalledWith('/')
        consoleErrorSpy.mockRestore()
    })
});